...
function NewSession({ members, user, calendarConnected, connectedEmails, demoMode, onCreated, onCancel }: {
  ...
  const formattedTitle = noProject ? details.title.trim() : `[${projectName.trim()}] ${details.title.trim()}`;
  const projectInvalid = (!noProject && (!projectName.trim() || /[\[\]\r\n]/.test(projectName))) || formattedTitle.length > 160;
  ...
  const submit = async () => {
    ...
    try {
      ...
      const created = await api.createRequest({ ...details, project_name: noProject ? "" : projectName.trim(), no_project: noProject, session_type: sessionType, start_at: slot.start_at, end_at: slot.end_at, participant_emails: selected, force: forceMode, timezone: SCHEDULE_TIMEZONE });
  ...
  const goBack = (target: number) => {
    ...
    if (target === 1) { setSlot(null); setSlots([]); }
    if (target === 0) onCancel();
    else setStep(target);
  };
  return <section className="wizard-page">
    <div className="page-heading"><div><button type="button" className="btn btn-ghost" disabled={loading && step === 3} onClick={() => goBack(step - 1)}><ChevronLeft size={17} /> {step === 1 ? "Retour au tableau de bord" : step === 2 ? "Modifier l’équipe, la date ou la durée" : "Retour aux créneaux"}</button><h1>Nouvelle session</h1><p>Organisez un temps de travail avec votre équipe.</p></div><div className="steps">{[1, 2, 3].map((number) => <div key={number} className={number <= step ? "active" : ""}><span>{number < step ? <Check size={13} /> : number}</span><label>{["Équipe", "Créneau", "Détails"][number - 1]}</label></div>)}</div></div>
    {step === 3 && <div className="wizard-footer"><button type="button" className="btn btn-ghost" disabled={loading} onClick={() => goBack(2)}><ChevronLeft size={17} /> Modifier le créneau</button><button type="button" className="btn btn-ghost" disabled={loading} onClick={() => goBack(1)}>Modifier la date, la durée ou l’équipe</button></div>}
