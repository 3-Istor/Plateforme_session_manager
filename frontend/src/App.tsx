...
type View = "dashboard" | "new" | "requests" | "lateness";
...
function LatenessPage({ entries, members, manager, onSave }: {
  ...
  const save = async (email: string) => {
    ...
    if (!rawPoints || !Number.isInteger(points) || points < 0 || points > 1_000_000) { setLocalError("Le nombre de points doit être un entier positif ou nul (maximum 1 000 000)."); return; }
  ...
  return <section className="lateness-page"><div className="page-title"><div><span className="eyebrow">CLASSEMENT DE L'ÉQUIPE</span><h1>Points de retard</h1><p>{manager ? "Modifiez les points de chaque membre. Le classement est visible par toute l'équipe." : "Consultez le classement des retards de l'équipe."}</p></div><span className="ranking-icon"><Trophy size={24} /></span></div>
    ...
    <div className={`ranking-panel ${manager ? "manager" : "viewer"}`}><div className="ranking-head"><span>Classement</span><span>Membre</span><span>Points de retard</span>{manager && <span>Action</span>}</div><div className="ranking-list">{ranking.map((entry, index) => {
