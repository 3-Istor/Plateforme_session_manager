...
`GOOGLE_TARGET_CALENDAR_ID` est obligatoire : l'application ne crée plus d'agenda secondaire et refuse la connexion Calendar du manager si la cible est absente. Pour cette instance, la cible est **Agenda 3-ISTOR** (`913d31d1666fcfd7a8ad0c6447679a0ceafdbff55a20adb5a2dea1f655f39c92@group.calendar.google.com`). Le compte défini par `MANAGER_EMAIL` doit avoir le droit de modifier ses événements, directement ou via le groupe auquel l'agenda est partagé. Lors de la connexion, l'application vérifie que le rôle renvoyé par Google est bien `writer` ou `owner` sur cet agenda exact. Après toute modification de la cible ou des permissions, reconnectez l'agenda du manager.
